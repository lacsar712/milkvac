...
import (
	...
	"github.com/lacsar712/milkvac/internal/cluster"
	"github.com/lacsar712/milkvac/internal/milklane"
	"github.com/lacsar712/milkvac/internal/fsm"
...
type App struct {
	...
	plant        *milklane.ParlorPlant
	towerFSM     *fsm.TowerFSM
	zones        *milklane.ZoneTable
	...
	holdMgr      *vacuum.HoldWindowManager
	holdEval     *vacuum.HoldWindowEvaluator
	gradAudit    *vacuum.OrderedGradientValidator
	guard        *interlock.Guard
	dryFSM       *fsm.ParlorFSM
	scheduler    *clock.SegmentScheduler
	avgWindow    *clock.PulseWindow
...
func New(cfg config.Config) (*App, error) {
	...
	zones, err := milklane.NewZoneTable(towerID, cfg.ZoneCount, plenumID, cfg)
	if err != nil {
		return nil, err
	}
	plant := milklane.NewParlorPlant(cfg, clk, mem)
	...
	for i, z := range zones.Zones() {
		...
		plant.RegisterSensor(vacuum.NewSensor(sensorID, z.Zone))
		targets = append(targets, cfg.TargetMoistPct)
	}
	if err := plant.InitProfile(zoneIDs, targets); err != nil {
		return nil, err
	}
	ventBank, err := cluster.NewVentBank(zoneIDs, "vent-damper")
	if err != nil {
		return nil, err
	}
	...
	holdMgr := vacuum.NewHoldWindowManager()
	a := &App{
		...
		gradAudit:    vacuum.NewOrderedGradientValidator(zoneIDs, cfg.MaxGradientDeltaPct, 0.5),
		guard:        interlock.NewGuard(guardPairs),
		batchCancels: make(map[model.TowerID]context.CancelFunc),
	}
	a.cluster = initDehumidController(ventBank, cfg.MaxGradientDeltaPct, func(readings []model.MoistureReading) float64 {
	...
	if pc, ok := a.clk.(*clock.ProcessClock); ok {
		...
		a.avgWindow = clock.NewPulseWindow(a.clk, time.Duration(cfg.PulseWindowMinutes)*time.Minute)
	}
	a.dryFSM = fsm.NewParlorFSM(towerID, a.onDryTransition)
	fsm.RegisterDryHeatHook(a.dryFSM.Hooks())
	a.dryRamp = NewVacRamp(a.clk, cfg.ProcessTick(), cfg.VacRampSteps)
