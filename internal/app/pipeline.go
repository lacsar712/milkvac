...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.cluster.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.cluster.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.cluster.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *vacuum.ProfileManager, windows *vacuum.HoldWindowManager) *vacuum.HoldWindowEvaluator {
	return vacuum.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *cluster.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *cluster.Controller {
	c := cluster.NewController(bank, maxDelta)
